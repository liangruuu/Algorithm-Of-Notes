...
// 求出arr[l...r]范围里第k小的数
template <typename T> int __selection(T arr[], int l, int r, int k) {
...
// 如果希望我们的算法中k的语意是从1开始的, 只需要在整个逻辑开始进行k--即可,
// 可以参考selection2
template <typename T> int selection(T arr[], int n, int k) {
...
template <typename T> int selection2(T arr[], int n, int k) {
